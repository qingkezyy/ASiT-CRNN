...
import torchaudio
from desed_task.audiossl.transforms.common import Normalize,MinMax,RandomCrop,Identity,CentralCrop
from desed_task.audiossl.transforms.byol_a import Mixup, RandomResizeCrop
from desed_task.audiossl.transforms.target_transform import MixupSpecLabel,MixupSpecLabelAudioset
...
class FinetuneTrainTransform:
    def __init__(self,sr=16000,max_len=12):
        melspec_t = torchaudio.transforms.MelSpectrogram(
            sr, f_min=0, f_max=8000, hop_length=270, win_length=1024, n_fft=1024, n_mels=128)
        to_db = torchaudio.transforms.AmplitudeToDB(stype="power",top_db=80)
        normalize = MinMax(min=-79.6482,max=50.6842)
        self.sr=sr
        self.len=len
