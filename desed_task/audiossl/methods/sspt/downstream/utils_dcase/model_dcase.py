...
from desed_task.audiossl.datasets.dcase_utils import ManyHotEncoder
from desed_task.audiossl.utils.common import cosine_scheduler_epoch
from desed_task.audiossl.methods.sspt.downstream.utils_dcase.class_dict import classes_labels
from desed_task.audiossl.methods.sspt.downstream.utils_psds_eval.gpu_decode import (
...
from desed_task.audiossl.methods.sspt.downstream.utils_psds_eval.evaluation import (
...
class FineTuningPLModule(LightningModule):
    def __init__(self,
        ...
        self.num_labels = num_labels
        self.loss_fn = torch.nn.BCELoss() ## torch.nn.MSELoss()
        self.monitor = 0
    ...
    #     for n, p in self.encoder.encoder.v.patch_embed.named_parameters():
    ...
    #         else:
    #             print(p.grad.norm())
    #     for i, layer in enumerate(self.encoder.encoder.v.blocks):
    #         print(f"Layer {i}")
