...
import desed_task.audiossl.methods.sspt.downstream.comparison_models.models.vision_transformer as vits
...
class ASTModel(nn.Module):
    def __init__(self, finetune_from, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, patch_size=16, model_name='vit_base', verbose=True, keyword='teacher'):
        ...
        ## self.v.feat_mean = nn.AvgPool2d([8, 1], padding=[1, 0])
        self.v.feat_mean = nn.AvgPool1d(8, 8)
    ...
    def forward(self, x):
        ...
        x = x[:, 1:, :]
        x = self.v.norm(x)
        x = self.v.feat_mean(x.transpose(-1, -2)).transpose(-1, -2)
        
        ## x = x[:, 1:, :].reshape(B, 8, -1, 768)
        # average output of all tokens except cls token(s)
        ## x = x.permute(0, 3, 1, 2)
        ## x = self.v.feat_mean(x)
