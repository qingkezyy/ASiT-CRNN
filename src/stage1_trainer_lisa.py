...
from desed_task.data_augm import mixup, mixup_w_pretrained
...
class DynamicLayerActivationCallback(Callback):
    def __init__(self, n_layers, interval_steps, model):
        ...
        self.layers_attribute = 'asit.asit.v.blocks'
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx): 
        ...
        # elif step_num % 13200 == 0:
        #     layers = eval('self.sed_student.asit.asit.v.blocks')
        #     for i in 12:
        #         for param in layers[i].parameters():
        #             param.requires_grad = True

    # def on_train_batch_end(self, trainer, pl_module, batch, batch_idx, unused):
    #     # 清理资源
    #     self.cleanup()

    # def cleanup(self):
    #     # 在这里进行资源清理操作，例如关闭文件、释放内存等
    #     pass
...
class SEDTask4(pl.LightningModule):
    ...
    def training_step(self, batch, batch_indx): ## batch就是StronglyAnnotatedSet.__getitem__中的输出out_args？
        audio, asit_feats, labels, padded_indxs, _ = batch
        ...
        if mixup_type is not None and 0.5 > random.random():
            weak_feats, labels_weak = mixup(
                sed_feats[weak_mask], labels_weak, mixup_label_type="soft"
            )
            strong_feats, labels[strong_mask] = mixup(
                sed_feats[strong_mask], labels[strong_mask], mixup_label_type="soft"
            )
            unlabeled_feats, _, = mixup(
                sed_feats[unlabelled_mask], labels[unlabelled_mask], mixup_label_type="soft"
            )
        else:
            weak_feats = sed_feats[weak_mask]
            strong_feats = sed_feats[strong_mask]
            unlabeled_feats = sed_feats[unlabelled_mask]

        sed_feats = torch.concat([strong_feats, weak_feats, unlabeled_feats], dim=0)

        current_step = self.scheduler[0]["scheduler"].step_num
        if current_step % 5 == 0:
            layers = eval('self.sed_student.asit.asit.v.blocks')  # Dynamically execute to get layers
            
            for num_layers in range(len(layers)):
                for param in self.sed_student.asit.asit.v.blocks[num_layers].parameters():
                        param.requires_grad = False
            # torch.cuda.empty_cache()

            active_layers_indices = np.random.choice(12, 2, replace=False)
            # print(f"Activating layers at indices: {active_layers_indices} for the next steps.", flush=True)
            # Enable gradients only for the selected layers
            for idx in active_layers_indices:
                for param in self.sed_student.asit.asit.v.blocks[idx].parameters():
                    param.requires_grad = True

        # sed student forward
        strong_preds_student, weak_preds_student = self.detect(
            sed_feats, asit_feats, self.sed_student,
        )
        ...
        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher = self.detect(
                sed_feats, asit_feats, self.sed_teacher
            )
    ...
    def validation_step(self, batch, batch_indx):
        audio, asit_feats, labels, padded_indxs, filenames = batch

        sed_feats = self.mel_spec(audio)
        # prediction for student
        strong_preds_student, weak_preds_student = self.detect(sed_feats, asit_feats, self.sed_student)
        # prediction for teacher
        strong_preds_teacher, weak_preds_teacher = self.detect(sed_feats, asit_feats, self.sed_teacher)
