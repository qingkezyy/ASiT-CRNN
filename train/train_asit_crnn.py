...
from desed_task.nnet.CRNN_asit import CRNN
from local.scheduler import ExponentialWarmup
from local.classes_dict import classes_labels
from local.stage1_trainer import SEDTask4
from desed_task.dataio.datasets_asit_sed import StronglyAnnotatedSet, UnlabeledSet, WeakSet
...
def single_run(
    ...
    sed_student = CRNN(
        unfreeze_asit_layer=config["opt"]["tfm_trainable_layers"], 
        **config["net"], 
        model_init=config["ultra"]["model_init"],
        asit_init=config["ultra"]["asit_init"],)
...
def prepare_run(argv=None):
    ...
    parser.add_argument(
        "--conf_file",
        default="./confs/asit_crnn.yaml",
        help="The configuration file with all the experiment parameters.",
    )
    parser.add_argument(
        "--log_dir",
        default="./exp/asit_crnn/",
