...
from desed_task.nnet.CRNN_ssl import CRNN
from local.scheduler import ExponentialWarmup
from local.classes_dict import classes_labels
from local.ssl_crnn_trainer import SEDTask4
from desed_task.dataio.datasets_ssl_sed import StronglyAnnotatedSet, UnlabeledSet, WeakSet
...
def single_run(
    ...
    encoder = ManyHotEncoder(
        ...
        frame_hop=config["feats"]["hop_length_encoder"],
    ...
    sed_student = CRNN(
        unfreeze_encoder_layer=config["opt"]["tfm_trainable_layers"], 
        **config["net"], 
        encoder_init=config["ultra"]["encoder_init"],)
...
def prepare_run(argv=None):
    ...
    parser.add_argument(
        "--conf_file",
        default="./confs/ssl_crnn.yaml",
        help="The configuration file with all the experiment parameters.",
    )
    parser.add_argument(
        "--log_dir",
        default="./exp/ssl_crnn/",
