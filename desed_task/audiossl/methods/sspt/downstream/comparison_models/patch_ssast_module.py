...
from desed_task.audiossl.methods.sspt.downstream.comparison_models.models import ASTModel
...
class SSASTModel(ASTModel):
    ...
    def forward(self, x): ##x: torch.Size([256, 998, 128])
        x = x.unsqueeze(1)
        x = x.transpose(2, 3)
        # Rewrite ASTModel forward
        B = x.shape[0]
        x = self.v.patch_embed(x) ## x: torch.Size([256, 496, 768])
...
if __name__ == '__main__':
    input_tdim = 1024
    fake_x = torch.zeros([1, input_tdim, 128])
    fake_x[:, 16:32, :] = 1
    model = PatchSSASTPredModule("/public/home/03455/ZYY/ssast-main/pretrained_model/SSAST-Base-Patch-400.pth", "dcase")
    output = model(fake_x)
