...
from desed_task.audiossl.methods.sed.fairseq import utils
from desed_task.audiossl.methods.sed.fairseq.data.data_utils import compute_mask_indices
from desed_task.audiossl.methods.sed.fairseq.dataclass import ChoiceEnum, FairseqDataclass
from desed_task.audiossl.methods.sed.fairseq.models import BaseFairseqModel
from desed_task.audiossl.methods.sed.fairseq.modules import GradMultiply, LayerNorm
...
@dataclass
class MAE_AST_TASK_Config(FairseqDataclass):
...
if __name__ == "__main__":
    ...
    load_weigts = torch.load("./ckpts/random_frame_75_12LayerEncoder.pt")
    state_dicts = load_weigts["model"]
    model = MAE_AST()
    for layer in model.encoder.layers:
        print(layer)
    model.load_state_dict(state_dict=state_dicts, strict=True)
    fake_input = torch.rand(10, 998, 128)
    fake_output, pad_output = model.extract_features(fake_input, padding_mask=torch.FloatTensor(10, 998).fill_(0).bool())
    print(fake_output.shape, fake_output.sum())